  ...
  <section>
    ...
    <div class="request-form" v-show="showForm" style="padding: 20px 0">
      <div class="flex">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          style="margin: 5px 0 10px 10px;color: var(--post);"
          width="30"
          height="30"
          viewBox="0 0 24 24"
        >
      ...
      <section>
        <div class="post-sub-bar sub-bar" @click="showReqBody = !showReqBody">
          <div class="flex">
            <svg
      ...
      <hr style="color: var(--post-m); border-color: var(--post-m);" />
      <button
        type="submit"
        style="--_background: var(--post-gradient)"
    ...
    <div class="all-responses" :key="responses.length">
      ...
      >
        ...
        <div class="response-main" v-show="oneResponse.show">
          <section>
            <div
              class="post-sub-bar sub-bar sub-mid-bar"
...
async function doRequest(index) {
  ...
  if (url.value === "") {
    document.getElementById("urlInputPost").style.outline = "3px solid red";
    setTimeout(() => {
      document.getElementById("urlInputPost").style.outline = "";
    }, 3000);
    return;
  }
