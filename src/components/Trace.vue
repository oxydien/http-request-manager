<template>
  <div class="error">Trace request functionality needs your contribution to improve</div>
  <section>
    <div class="trace-bar bar">
    ...
    <div class="request-form" v-show="showForm" style="padding: 20px 0">
      <div class="flex">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          style="margin: 5px 0 10px 10px;color: var(--trace);"
          width="30"
          height="30"
          viewBox="0 0 24 24"
        >
      ...
      <hr style="color: var(--trace-m); border-color: var(--trace-m);" />
      <button
        type="submit"
        style="background: var(--trace-gradient)"
    ...
    <div class="all-responses" :key="responses.length">
      ...
      >
        ...
        <div class="response-main" v-show="oneResponse.show">
          <section>
            <div
              class="trace-sub-bar sub-bar sub-mid-bar"
          ...
          <section
            ...
            v-show="
              getContentTypeHeaderValues(oneResponse.headers).some((value) =>
                value.toLowerCase().toString().includes('text/htm')
              )
            "
          >
            <div
              class="trace-sub-bar sub-bar sub-mid-bar"
              @click="oneResponse.showPreview = !oneResponse.showPreview"
            >
              <div class="flex">
                <svg
                  :class="oneResponse.showPreview ? 'open' : ''"
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                  viewBox="0 0 24 24"
                ...
                Response Preview (danger)
              </div>
            </div>
            <iframe
              :srcdoc="oneResponse.text"
              v-if="oneResponse.showPreview"
              frameborder="0"
            ></iframe>
          </section>
        </div>
...
async function doRequest(index) {
  ...
  if (url.value === "") {
    document.getElementById("urlInputTrace").style.outline = "3px solid red";
    setTimeout(() => {
      document.getElementById("urlInputTrace").style.outline = "";
    }, 3000);
    return;
  }
  ...
  try {
    const headersArray = deleteValidHeaders();
...
function getContentTypeHeaderValues(headers) {
